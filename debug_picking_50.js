...
function fetchTrendyol(sellerId, auth, status) {
    const options = {
        ...
        port: 443,
        path: `/integration/order/sellers/${sellerId}/orders?status=${status}&size=50&orderByField=OrderDate&orderByDirection=DESC`,
        method: 'GET',
        headers: {
...
async function main() {
    try {
        ...
        console.log('--- RECENT PICKING ORDERS (TOP 50) ---');
        const data = await fetchTrendyol(sellerId, auth, 'Picking');
        if (data.content) {
            data.content.forEach((o, i) => {
                console.log(`${i + 1}. Order: ${o.orderNumber} | Date: ${new Date(o.orderDate).toLocaleString('tr-TR')} | Status: ${o.status}`);
            });
        }
    } catch (e) {
