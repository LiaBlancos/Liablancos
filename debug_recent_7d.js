...
function fetchTrendyol(sellerId, auth, status, startDate) {
    let path = `/integration/order/sellers/${sellerId}/orders?status=${status}&size=50&orderByField=OrderDate&orderByDirection=DESC`;
...
async function main() {
    try {
        ...
        console.log('--- RECENT ORDERS (LAST 7 DAYS) ---');
        const statuses = ['Created', 'Picking', 'Shipped'];
        for (const status of statuses) {
            const data = await fetchTrendyol(sellerId, auth, status, sevenDaysAgo);
            console.log(`[${status}] Count: ${data.totalElements}`);
            if (data.content) {
                data.content.forEach(o => {
                    console.log(`  Order: ${o.orderNumber} | Date: ${new Date(o.orderDate).toLocaleString('tr-TR')} | Status: ${o.status}`);
                });
            }
        }
    } catch (e) {
