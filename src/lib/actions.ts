...
export async function getIncomes() {
    ...
    return data.map(d => ({
        ...
        stokTakipli: d.stok_takipli,
        indirimTutari: d.indirim_tutari,
...
export async function bulkSaveIncomes(dataList: any[]) {
    ...
    // Use upsert with islem_no to prevent duplicate imports
    const hasIslemNo = payload.some(p => p.islem_no);
    if (hasIslemNo) {
        const { error } = await supabase.from('incomes').upsert(payload, { onConflict: 'islem_no', ignoreDuplicates: true });
        if (error) throw new Error(error.message);
    } else {
...
export async function bulkDeleteIncomes(ids: string[]) {
    const { error } = await supabase
        .from('incomes')
        .delete()
        .in('id', ids);
...
export async function bulkUpdateIncomeStatus(ids: string[], status: string) {
    const { error } = await supabase
        .from('incomes')
        .update({ tahsilat_durumu: status })
        .in('id', ids);
